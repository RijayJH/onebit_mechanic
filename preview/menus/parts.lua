local getModLabel = require('preview.utils.getModLabel')
local originalMods = {}
partsLastIndex = 1
local VehicleClass = require('preview.utils.enums.VehicleClass')
...
local function parts()
    ...
    if GetVehicleClass(vehicle) ~= VehicleClass.Cycles then
        options[#options + 1] = {
            ...
            close = true,
            args = {
                menu = 'preview.menus.wheels',
            }
        }
    end

    options[#options + 1] = require('preview.options.plateindex')()
...
local function onSubmit(selected, scrollIndex, args)
    ...
    local success = require('preview.utils.installMod')(duplicate, 'cosmetic', {
        description = desc,
    })
